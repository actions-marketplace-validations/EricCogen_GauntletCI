...
public static class CorpusCommand
{
        ...
        cmd.SetHandler(async (ctx) =>
        {
            ...
            using (db)
            {
                ...
                foreach (var c in candidates)
                {
                    ...
                    insertCmd.Parameters.AddWithValue("$createdAt", c.CreatedAtUtc == default ? DBNull.Value : c.CreatedAtUtc.ToString("O"));
                    insertCmd.Parameters.AddWithValue("$updatedAt", c.UpdatedAtUtc == default ? DBNull.Value : c.UpdatedAtUtc.ToString("O"));
    ...
    private static async Task<int> SeedCodeScanningCandidatesAsync(
        HttpClient http, SqliteConnection conn,
        string owner, string repo, int limit, CancellationToken ct)
    {
        ...
        var json = await resp.Content.ReadAsStringAsync(ct);
        JsonElement[] prs;
        try { prs = JsonSerializer.Deserialize<JsonElement[]>(json) ?? []; }
        catch (Exception ex)
        {
    ...
    private static Command CreateMaintainersFetch()
    {
        ...
        cmd.SetHandler(async (ctx) =>
        {
            ...
            var targets = Corpus.MaintainerFetcher.MaintainerTarget.Defaults.ToList();
            foreach (var r in extraRepos)
            {
                var parts = r.Split('/', 2);
                if (parts.Length == 2)
                    targets.Add(new Corpus.MaintainerFetcher.MaintainerTarget(
            ...
            using var fetcher = Corpus.MaintainerFetcher.MaintainerFetcher.CreateDefault();
